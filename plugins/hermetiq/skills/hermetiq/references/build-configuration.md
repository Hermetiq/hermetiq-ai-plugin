...
|------|-----------------|----------------------|
| `--jobs=<N>` | Bazel's maximum concurrent actions; it is not remote worker capacity | Use only an explicit numeric value from a complete `get_invocation(includeCommandLine=true)` result, applying last-wins semantics. `get_build_parallelism` counts executing remote actions, so a peak below `--jobs` is a graph signal only when remote queue wait and scheduler backlog are low. High queueing with low executing parallelism means worker/scheduler capacity, and hitting `--jobs` while queueing is high is not a reason to raise it. Treat absent/`auto`/formula/truncated values as an unknown numeric cap. |
